...
function initData(vueOptions, context) {
  ...
  if (typeof data === 'function') {
    try {
      ...
      if (Object({"NODE_ENV":"development","VUE_APP_NAME":"2019nCoV","VUE_APP_PLATFORM":"mp-weixin","BASE_URL":"/"}).VUE_APP_DEBUG) {
    ...
    if (eventsArray && isMatchEventType(eventType, type)) {
      eventsArray.forEach(function (eventArray) {
        ...
        if (methodName) {
          ...
          var params = processEventArgs(_this.$vm, event, eventArray[1], eventArray[2], isCustom, methodName); // 参数尾部增加原始事件对象用于复杂表达式内获取额外数据
